...
annEPIC <- readRDS("intermediate/annEPIC.rds")
...
# data set is small so use top 10000 genes
# sample size is small (<20) so use pairwise gene selection

#save plots to pdf
pdf("../results/MDS_before.pdf")

par(mfrow = c(1, 1))
plotMDS(getM(mSetFun), top=10000, gene.selection="pairwise",
...
keep <- !(featureNames(mSetFunFlt) %in% annEPIC$Name[annEPIC$chr %in%
                                                       c("chrX", "chrY")])
table(keep)
...
xloci <- xreactive_probes(array_type = "EPIC")
length(xloci) 
...
par(mfrow = c(1, 1))
plotMDS(getM(mSetFunFlt), top=10000, gene.selection="pairwise",
        col=pal[factor(targets$Sample_Type)])
legend("topleft", legend=levels(factor(targets$Sample_Type)), text.col=pal,
       bg="white", cex=0.7)
